...
// This would be the typical flow when a user interacts with the extension

console.log("=== ThinkDSA AI Score System Demo ===");

// Example AI response with score assessment
const exampleAIResponse = `
Great start! I can see you're thinking about using a hash map to solve this Two Sum problem. 

Here's a hint: Instead of checking if the complement exists after adding the current number to the map, try checking for the complement first. This way, you avoid the case where a number pairs with itself.
...
Consider what happens with edge cases like an empty array or when no solution exists.
